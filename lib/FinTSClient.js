    ...
    me.MsgInitDialog = function(cb){
	   ...
	   me.SendMsgToDestination(msg,function(error,recvMsg){
			if(error){
				...
				if(HIRMG!=null&&(HIRMG.getEl(1).getEl(1)=="0010"||HIRMG.getEl(1).getEl(1)=="0020"||HIRMG.getEl(1).getEl(1)=="3060")){
					if(Helper.checkMsgsWithBelongToForId(recvMsg,HKVVB.nr,"0020")||HIRMG.getEl(1).getEl(1)=="0020"){
					try{
						...
						if(me.konten.length==0){
							...
							for(var i=0;i!=konto_list.length;i++){
								...
								var el1 = konto_list[i].getEl(1);
								if (el1) {
									konto.konto_nr		= el1.getEl(1);
									konto.unter_konto	= el1.getEl(2);
									konto.ctry_code 	= el1.getEl(3);
									konto.blz			= el1.getEl(4);
								}
